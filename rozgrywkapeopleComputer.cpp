...
void rozgrywkaSinglePlayer(char who, char what)
{
    ...
    while (1)
    {
        ...
        if (zdarzenie.type == ALLEGRO_EVENT_KEY_UP)
        {
            ...
            if(zdarzenie.keyboard.keycode == ALLEGRO_KEY_ENTER && kolej=='w' && (postep==true || beatseries>0))
            {
                bijacy=NULL;
                kolej='b';
                postep=false;
        ...
        {
            if(klik &2 && kolej=='w' && (postep==true || beatseries>0))
            {
                bijacy=NULL;
                kolej='b';
                postep=false;
        ...
        if (kolej=='w' && zdarzenie.type == ALLEGRO_EVENT_MOUSE_AXES)
        {
    ...
    if(abilityToSave)
    {
        ...
        if (kolej=='b')
        {
            button=al_show_native_message_box(okno,"koniec gry","czarne wygraly","czy zaczac od nowa",NULL,ALLEGRO_MESSAGEBOX_YES_NO);
            clear_all(pole,pawn,okno,kolejka);
            if(button==1) rozgrywkaSinglePlayer('w','n');
        }
        else if(kolej=='w')
        {
            button=al_show_native_message_box(okno,"koniec gry","biale wygraly","czy zaczac od nowa",NULL,ALLEGRO_MESSAGEBOX_YES_NO);
            clear_all(pole,pawn,okno,kolejka);
            if(button==1) rozgrywkaSinglePlayer('b','n');
        }
    ...
    else
    {
        ...
        {
            FILE * Zapis;
            Zapis=fopen("zapisCompVsPlayer.txt","wt");
            fprintf(Zapis,"%c\n",kolej);
